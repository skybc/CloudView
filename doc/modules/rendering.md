...
## 概述
PointCloudViewer 是 CloudView.Controls 项目中的核心渲染控件，负责通过 OpenGL 和 Silk.NET 实现三维点云的实时可视化，并提供多 ROI 线框编辑与点云筛选/统计/裁剪能力。
...
### 4. ROI（感兴趣区域）编辑与业务计算 ✨ 新增
- 通过 `Rois` 依赖属性接收外部传入的 `IList<RoiBase>`
- 已支持 ROI 类型：`BoxRoi`、`SphereRoi`、`CylinderRoi`、`ConeRoi`
- 所有 ROI 统一采用线框表现，不做实心填充，避免遮挡点云
- 活动 ROI 显示实心立体单向箭头控制点和环形箭头旋转辅助环，可用左键拖拽实现移动、缩放、旋转
- 控制手柄使用全不透明实体渲染，不叠加可见描边线，降低锯齿和杂边感
- 未选中 ROI 仅显示基础线框，不显示任何控制点或旋转箭头
- 支持基于活动 ROI 的点云筛选、统计、裁剪：
  - `FilterActiveRoi()`
  - `CalculateActiveRoiStatistics()`
  - `CropActiveRoi(RoiCropMode mode)`
- 触发 `RoiSelected`、`RoiResultsChanged`、`ActiveRoiChanged`、`RoiEdited` 事件，向宿主同步 ROI 结果
...
**摄像机平移系统** ✨ 新增
- 右键拖动实现视野平移，左右上下移动
- 平移速度基于真实的相机-目标点距离和视口高度计算，手感在不同 FOV / 分辨率下更一致
- 平移后，摄像机位置与目标点会同步移动，保持相对关系不变
- 平移后同步旋转基准，避免下一次左键旋转沿用旧欧拉角状态
- 平移过程中不反推摄像机位置，避免累积误差、抖动和漂移

**平移实现细节**：
- `_isPanning`: 标志右键是否被按下
- `_panOffset`: 记录累积的平移偏移量（用于重置视图等操作的参考）
- 平移公式：`worldPerPixel = 2 * tan(fov / 2) * distance / viewportHeight`
- 使用稳定世界上方向 `Vector3.UnitY` 计算相机坐标系：
  - `forward`: 摄像机看向方向
  - `right`: `cross(forward, worldUp)`，当长度过小时回退到 `Vector3.UnitX`
  - `up`: `cross(right, forward)`
- 平移向量：`move = (-right * deltaX + up * deltaY) * worldPerPixel`
- 平移后同步更新 `_cameraPosition += move` 与 `_cameraTarget += move`
- 平移后同步 `_rotationX/_rotationY`，保证后续 Orbit 仍围绕当前视野中心稳定旋转
...
- 摄像机位置 = 目标点 + 旋转后的方向向量
- 俯仰角增加极点保护（约 89°），避免拖动到顶部/底部时出现翻转
...
鼠标左键：
  OnMouseLeftButtonDown() → 记录点击起点，并检查是否命中 ROI / 已选中 ROI 的控制点
    ↓
  未发生明显移动 → OnMouseLeftButtonUp() 时将命中的 ROI 设为选中状态；若点击空白区域则清空当前 ROI 选择
    ↓
  发生拖拽移动：
    - 若命中的是已选中 ROI 的控制点或本体 → 进入 Move / Resize / Rotate 编辑状态
    - 否则 → 执行视图旋转
    ↓
  选中 ROI 使用更粗的线框渲染，作为当前活动对象的视觉反馈
  直线方向编辑控制点使用实心“圆柱 + 圆锥”组合的单向箭头，旋转控制点使用独立贴附在旋转线上的实心“双向圆锥 + 圆柱 + 圆锥”环形箭头，提升识别度
  控制箭头采用全不透明实体渲染，不额外绘制可见边线；鼠标悬停或正在使用的控制箭头均会即时放大并高亮，提供更明确的交互反馈
  旋转拖拽按鼠标相对 ROI 屏幕中心的有符号角度驱动，屏幕顺时针拖拽对应视觉顺时针旋转，轴向朝向相机时无翻转，背向相机时自动取反

鼠标右键拖动平移视图：
  OnMouseRightButtonDown() → _isPanning=true
    ↓
  OnMouseMove() → 计算平移向量 → 更新 _cameraTarget/_cameraPosition → Render()
    ↓
  OnMouseRightButtonUp() → _isPanning=false
...
- `RefreshRois()`: 刷新 ROI 可视化与业务结果
- `FilterActiveRoi()`: 筛选活动 ROI 内的点
- `CalculateActiveRoiStatistics()`: 计算活动 ROI 内点云统计信息
- `CropActiveRoi()`: 基于活动 ROI 执行点云裁剪
...
### 事件
- `RoiSelected`: 活动 ROI 的筛选结果更新时触发
- `RoiResultsChanged`: ROI 筛选与统计结果更新时触发
- `ActiveRoiChanged`: 活动 ROI 切换时触发
- `RoiEdited`: ROI 被编辑完成时触发
...
- **实现细节**：
  - 新增 `_isPanning` 标志：表示右键是否被按下
  - 新增 `_panOffset` 字段：记录累积的平移偏移量
  - 在 `OnMouseRightButtonDown()` 和 `OnMouseRightButtonUp()` 中处理右键平移状态
  - 修改 `OnMouseMove()` 方法：添加平移处理逻辑

- **平移算法**：
  1. 获取摄像机的三个基向量（forward、right、up）
  2. 通过 `Vector3.Distance(_cameraPosition, _cameraTarget)` 获取真实距离
  3. 通过 FOV 和 viewportHeight 计算 `worldPerPixel`
  4. 根据鼠标移动距离计算平移向量：`move = (-right * deltaX + up * deltaY) * worldPerPixel`
  5. 同步更新摄像机与目标点：`_cameraPosition += move; _cameraTarget += move;`

- **稳定性处理**：
  - 使用 `Vector3.UnitY` 作为稳定世界上方向
  - 当 `right` 长度过小时回退到 `Vector3.UnitX`
  - 不再根据平移结果反推 `_cameraPosition`，从源头避免抖动和漂移
...
- **效果**：
  - 用户按住右键，拖动鼠标可以在屏幕空间中任意平移视野
  - 平移不会改变视角旋转状态，只改变视野中心
  - 平移不会改变缩放距离，保持一致的视野大小

### v2.7 - 多 ROI 线框编辑与点云业务计算
- **功能新增**：
  - 引入 `Rois` 依赖属性，支持外部传入多个 ROI
  - 新增 `BoxRoi`、`SphereRoi`、`CylinderRoi`、`ConeRoi` 四类业务 ROI
  - ROI 统一采用线框主体 + 实心控制手柄渲染，活动 ROI 显示轴向“圆柱 + 圆锥”单向箭头控制点与独立双向环形箭头旋转辅助环
  - 支持左键单击选择 ROI、单击空白区域取消选择、左键拖拽旋转视图、右键平移视图
  - 只有已选中的 ROI 才允许移动、缩放、旋转等编辑操作
  - 选中的 ROI 会以更粗的线条高亮显示；未选中 ROI 不显示任何编辑手柄
  - 当前被使用的控制箭头会进一步放大并高亮显示
  - 新增活动 ROI 的点云筛选、统计、裁剪方法和结果事件

- **实现细节**：
  - 新增 `PointCloudViewer.Roi.cs` partial 文件，承载 ROI 可视化、命中测试和交互状态机
  - ROI 内部复用 `LineSharp` 管线构建线框，但在渲染顺序上独立于 `Shapes`
  - 活动 ROI 的筛选结果会同步更新 `SelectedIndices` 和 `SelectedPoints`
  - 主程序 `MainWindow.xaml` / `MainWindow.xaml.cs` 新增 ROI 测试按钮，用于联调加载、筛选、统计、裁剪
