...
- 活动 ROI 显示控制点和三轴旋转辅助环，可用左键拖拽实现移动、缩放、旋转
...
鼠标左键：
  OnMouseLeftButtonDown() → 先做 ROI 命中测试（控制点 / 旋转环 / ROI 主体）
    ↓
  命中 ROI → 进入 Move / Resize / Rotate 编辑状态
    ↓
  未命中 ROI → 恢复原有视图旋转行为
    ↓
  OnMouseLeftButtonUp() → 若在 ROI 编辑中则提交编辑结果，否则结束视图旋转
...
- **功能新增**：
  ...
  - ROI 统一采用线框渲染，活动 ROI 显示控制点与旋转辅助环
  - 支持左键命中 ROI 时编辑、未命中时旋转视图，右键平移视图
